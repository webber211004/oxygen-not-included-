...
local function IsPowered(inst)
    return inst._powertask ~= nil and inst.components.trader:Enable()
end
...
local function OnHarvest(inst)--, node)

end

local function AbleToAcceptTest(inst, item, giver)
	if item.prefab == "kelp" then
        inst.AnimState:ClearOverrideSymbol("food")
		inst.AnimState:PushAnimation("work")
		inst:DoTaskInTime(4, function()
			inst.AnimState:PushAnimation("food")
			inst:DoTaskInTime(1, function()
				inst.AnimState:PlayAnimation("idle_food")
				
				inst:AddComponent("pickable")
				inst.components.pickable.picksound = "dontstarve/wilson/pickup_reeds"
				inst.components.pickable:SetUp("mushbar")
				inst.components.pickable.makeemptyfn = OnHarvest(inst)
			end)
		end)
		return true
	elseif item.prefab == "butter" then
		inst.AnimState:PushAnimation("work")
		inst.AnimState:OverrideSymbol("food", "bars", "fieldration")
		inst:DoTaskInTime(4, function()
			inst.AnimState:PushAnimation("food")
			inst:DoTaskInTime(1, function()
				inst.AnimState:PlayAnimation("idle_food")
				...
				inst:AddComponent("pickable")
				inst.components.pickable.picksound = "dontstarve/wilson/pickup_reeds"
				inst.components.pickable:SetUp("mushbar_fieldration")
				inst.components.pickable.makeemptyfn = OnHarvest(inst)
			end)
		end)
		return true
	elseif item.prefab == "jammypreserves" then
		inst.AnimState:PushAnimation("work")
		inst.AnimState:OverrideSymbol("food", "bars", "fruitcake")
		inst:DoTaskInTime(4, function()
			inst.AnimState:PushAnimation("food")
			inst:DoTaskInTime(1, function()
				inst.AnimState:PlayAnimation("idle_food")
				...
				inst:AddComponent("pickable")
				inst.components.pickable.picksound = "dontstarve/wilson/pickup_reeds"
				inst.components.pickable:SetUp("mushbar_fruitcake")
				inst.components.pickable.makeemptyfn = OnHarvest(inst)
			end)
		end)
		return true
...
return Prefab("microbemusher", fn, assets, prefabs),
    MakePlacer("microbemusher_placer", "winona_catapult_placement", "winona_catapult_placement", "idle", true, nil, nil, nil, nil, nil, placer_postinit_fn)
